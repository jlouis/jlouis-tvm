...
%audience, treatment, thanks, level.
\newpage{}
...
\end{lem}
\begin{proof}
  TODO \fxnote{This proof is essenstially written down in Twelf}
\end{proof}
...
\begin{gather*}
  ...
  \Phi;\Psi;\Gamma \tpbb BB' : \Psi'}
  {\Phi;\Psi;\Gamma \tpbb (bb \mapsto \dbundle, BB') : (bb \mapsto
    \sbundle, \Psi')}
\end{gather*}
...
\begin{lem}{[Weakening, Basic block wellformedness]}
  ...
  wellformedness. Formally, if $\Phi;\Psi;\Gamma \tpbb BB : \Psi'$
  then we can weaken $\Psi$ to $\Psi[bb \mapsto \sbundle]$ and then
  \begin{equation*}
    \Phi;\Psi[bb\mapsto \sbundle];\Gamma \tpbb BB : \Psi'
  \end{equation*}
\end{lem}
\begin{proof}
  ...
  can weaken the premise $\Phi;\Psi;\Gamma \tpbb BB' : \Psi'$ by the
...
\fixme{Cleanup source code for report inclusion, Move this mark down
  when fixed for a subset}
% \section{Prelude}
% {\footnotesize
