...
\begin{lem}
  ...
  Support $\Phi;\Gamma \tpbb L : \Psi / \tau$ and $\Psi(l) =
  \sbundle$. Then for all $1 \leq k \leq m$ we have $\Phi;\Psi;\Gamma
  \tpb d_k : \sigma_k / \tau$.
\end{lem}
\begin{proof}
  Induction on the derivation $\Phi;\Gamma \tpbb L : \Psi /
  \tau$. Either we have a Wf-bb/s rule with a conclusion of the form
  $\Phi;\Gamma \tpbb L : (l \mapsto \sbundle, \Psi') / \tau$ then the
  premises of that rule is exactly what we seek.

  Otherwise, the conclusion is of the form $\Phi;\Gamma \tpbb L : (l'
  \mapsto \dotsb, \Psi') / \tau$ with $l \neq l'$. In that case, we must have
  $\Psi'(l) = \sbundle$, so we can apply the induction hypothesis with
  $\Phi;\Gamma \tpbb L : \Psi' / \tau$. This yields a proof of
