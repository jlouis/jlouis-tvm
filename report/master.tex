...
\fixme{Fresh variables --- think about the consequence}
\fixme{In the lemmas, make sure we mention something to the like of
  $l' \not \in \dom{\Psi}$}
...
\begin{figure}
  ...
  \begin{gather*}
    ...
    \inference[S-brc-t]{}{D;L |-
      \ibrc{\btrue}{l^k}{(c_1, \dotsc, c_n)}{l^{k'}}{(c'_1, \dotsc, c'_n)} -> \ibr{l^k}{(c_1, \dotsc, c_n)}}\\\\
...
\begin{lem}{[Weakening, Definitions, $\Psi$]}
  ...
  : \sigma / \tau$ and for a fresh $l'$, $l' \mapsto \sbundle$, then
  $\Phi;\Psi[l' \mapsto \sbundle];\Gamma \tpb d : \sigma / \tau$.
\end{lem}
\begin{proof}
...
\begin{proof}
  ...
  \item (S-letrec-s):
    ...
    $l \not \in \dom{\Psi}$, we \fixme{Consider lemmaing this}
