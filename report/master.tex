...
its encoding in \twelf{}. Informally, we can transform a fragment
\begin{verbatim}
  %sub = sub i32 %n, 1
  %call = call i32 @tri(i32 %sub)
  ...
...
type $\tau_1 -> \dotsc -> \cdot$. The instruction is then well-typed
if all the formal parameters obeys the type scheme of the
$\phi$-nodes.
...
When encoding a system into \twelf{}, we must make certain choices
along the way. Almost any operational semantics will --- in the name of
notational simplicity --- omit specific parts. The encoding needs to
take care of this explicitly, however.
...
\begin{lem}
  ...
  Suppose $D |- \Phi : D$, $D;\Psi;\Gamma |- i : \tau$ and $D;\Psi;\Gamma |- i ->
...
\begin{proof}
  ...
  \item (Branch) If the step is of the form
    \begin{equation*}
     \inference{BB(bb) = Ks \quad Ks(k) = i(x_1, \dotsc, x_n)}{\Phi;BB |- \ibr{bb^k}{(c_1, \dotsc, c_n)} ->
      i(c_1, \dotsc, c_n)}
    \end{equation*}
    then we must have the following type derivation
    \begin{equation*}
    \inference{\Psi(bb^n) = \tau_1 -> \tau_2 -> \dotsc ->
      \tau_n -> \cdot\\
      c_1 : \tau_1 \quad c_2 : \tau_2 \quad \dotsb \quad c_n : \tau_n}
    {D;\Psi;\Gamma |- \ibr{bb^k}(c_1, c_2, \dotsc, c_n) : \tau}
    \end{equation*}
